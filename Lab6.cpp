...
namespace {
    ...
    int OptimalPageReplacement(const vector<int> &trace,
        ...
        vector<int> frames;
        vector<int> frame_access_time;
        int i;
        for(i = 0; i < frameCount; ++i){
            frames.push_back(frameUsage[i*trace.size() + traceIndex]);
            frame_access_time.push_back(0);
        }
        for(int f = 0; f < frameCount; ++f){
            i = traceIndex;
            while(i < trace.size() && trace.at(i) != frames.at(f)){ ++i; }
            frame_access_time.at(f) = i;
        }
        for(int i = 0; i < frameCount; ++i){
            if(frame_access_time.at(i) > frame_access_time.at(victimPageFrame)){
                victimPageFrame = i;
            }
        }
        //victimPageFrame = frames.at(victimPageFrame);
    ...
    int FIFOPageReplacement(const vector<int> &trace,
        ...
        for(int f = 0; f < frameCount; ++f){
            i = traceIndex;
            row = f*trace.size();
            frame_time.push_back(0);
            while(i >= 0 && frameUsage[row+i] == frameUsage[row+i-1]){
                ++frame_time.at(f);
                --i;
            }
        }
        for(int i = 0; i < frame_time.size(); ++i){
            if(frame_time.at(i) > frame_time.at(victimPageFrame)){
                victimPageFrame = i;
            }
        }
        return victimPageFrame; // return the page frame to replace
    ...
    int LRUPageReplacement(const vector<int> &trace,
        ...
        for(int i = 0; i < accessTime.size(); ++i){
            if(accessTime.at(i) < accessTime.at(victimPageFrame)){
                victimPageFrame = i;
            }
        }
        return victimPageFrame; // return the page frame to replace
